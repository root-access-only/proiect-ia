...
class TrafficVision:
    ...
    def _detect_traffic_lights(self, bgr, hsv, max_area) -> List[Detection]:
        ...
        out: List[Detection] = []
        # Accept lamps as small as ~25 px so we can pick lights up from a
        # distance — the dark-housing check still discriminates them from
        # other red blobs (stop, no_entry, speed-limit rim).
        light_min_area = max(20.0, self.min_area * 0.05)
        for color_name, mask in (
            ...
            for cnt in contours:
                area = cv2.contourArea(cnt)
                if area < light_min_area or area > max_area:
                    continue
                circ = _circularity(cnt)
                if circ < 0.5:
                    continue
                ...
                if not 0.55 < aspect < 1.8:
                    continue
                # Required: a fraction of surrounding pixels must be dark
                # (housing). This separates a traffic-light lamp from a red
                # stop/no-entry sign on a white plate, and works at any
                # distance because we check a *fraction* not the mean.
                if not self._has_dark_surround(bgr, x, y, w, h):
                    continue
                out.append(Detection(
                    label=f"traffic_light_{color_name}",
                    confidence=float(min(0.95, circ + 0.1)),
                    bbox=(int(x), int(y), int(w), int(h)),
    ...
    # Helpers
    # ------------------------------------------------------------------
    def _has_dark_surround(self, bgr, x, y, w, h, pad: int | None = None,
                            dark_thresh: float = 110.0,
                            min_dark_fraction: float = 0.18) -> bool:
        """True if enough of the surrounding pixels are dark (housing).

        Uses a *fraction* test instead of the mean so it survives heavy
        blur at distance: even a small lamp viewed from far away keeps
        some recognisable dark housing pixels in its neighbourhood, while
        a stop/no-entry sign on a white plate has essentially none.
        """
        if pad is None:
            # Generous, scale-relative ring so we sample real housing even
            # when the lamp is only a handful of pixels.
        ...
        if nonzero.size == 0:
            return False
        return float(np.mean(nonzero < dark_thresh)) >= min_dark_fraction
