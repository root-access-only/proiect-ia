...
class NavigationConfig:
    ...
    turn_duration_s: float = 6.0  # safety timeout if yaw reading fails
    turn_speed: float = 1.1  # max angular spin (rad/s); lower = less overshoot
    turn_speed_min: float = 0.08  # crawl speed near target so we can settle precisely
    turn_kp: float = 2.2  # proportional gain: spin = kp * remaining_angle
    turn_brake_pulse_s: float = 0.0  # deprecated; closed-loop correction replaces brake pulse
    turn_angle_rad: float = np.pi / 2  # target turn = 90 deg
    turn_angle_tol_rad: float = np.deg2rad(1.2)  # final precision target
    detection_min_area: int = 600
    detection_max_area_ratio: float = 0.5
    sign_min_conf: float = 0.55
    red_light_max_wait_s: float = 8.0  # cap WAIT_FOR_GREEN so robot never deadlocks
...
class NavigationController:
    ...
    def __init__(
        ...
        self._turn_target_delta: float = 0.0
        self._mandatory_consumed: set[Tuple[int, int]] = set()
        self._red_light_entered_at: float = 0.0
        self._running = False
    ...
    def _react_to_world(
        ...
        if self.state in (NavigationState.TURNING_LEFT, NavigationState.TURNING_RIGHT):
            delta = self._turn_delta()
            target = cfg.turn_angle_rad
            timed_out = now >= self._turn_until

            if timed_out:
                self.driver.set_velocity(0.0, 0.0)
                self._turn_start_yaw = None
                self._transition(NavigationState.CRUISE, "Viraj terminat (timeout) - reiau cruise", detections)
                return

            if delta is None:
                # No yaw feedback: spin open-loop, timeout will eventually stop us
                spin = cfg.turn_speed * 0.6
            ...
            # Closed-loop P-control with active overshoot correction.
            # error > 0 -> need to turn more in the chosen direction
            # error < 0 -> overshot; counter-spin until back to target
            error = target - abs(delta)

            if abs(error) <= cfg.turn_angle_tol_rad:
                self.driver.set_velocity(0.0, 0.0)
                self._turn_start_yaw = None
                final_deg = np.degrees(delta)
                self._transition(
                    NavigationState.CRUISE,
                    f"Viraj 90 grade terminat ({final_deg:.1f} grade)",
                    detections,
                )
                return

            spin = max(cfg.turn_speed_min, min(cfg.turn_speed, cfg.turn_kp * abs(error)))
            direction = 1.0 if error > 0 else -1.0  # negative = counter-spin to undo overshoot
            if self.state == NavigationState.TURNING_LEFT:
                self.driver.set_velocity(-direction * spin, direction * spin)
            else:
                self.driver.set_velocity(direction * spin, -direction * spin)
            return
        ...
        if self.state == NavigationState.WAIT_FOR_GREEN:
            ...
            elif now - self._red_light_entered_at >= cfg.red_light_max_wait_s:
                self._transition(NavigationState.SLOW, "Semafor: timeout asteptare - reiau cu prudenta", detections)
            else:
                self.driver.set_velocity(0.0, 0.0)
                return
        ...
        if red_light and red_light.confidence >= cfg.sign_min_conf:
            if self.state != NavigationState.WAIT_FOR_GREEN:
                self._red_light_entered_at = now
        ...
        if mand_right and mand_right.confidence >= cfg.sign_min_conf:
            ...
            if sig not in self._mandatory_consumed:
                ...
                self._transition(NavigationState.TURNING_RIGHT, "Mandatory RIGHT - virez dreapta 90 grade", detections)
                self._turn_until = now + cfg.turn_duration_s
                self._turn_start_yaw = self._safe_yaw()
        ...
        if mand_left and mand_left.confidence >= cfg.sign_min_conf:
            ...
            if sig not in self._mandatory_consumed:
                self._mandatory_consumed.add(sig)
                self._transition(NavigationState.TURNING_LEFT, "Mandatory LEFT - virez stanga 90 grade", detections)
                self._turn_until = now + cfg.turn_duration_s
                self._turn_start_yaw = self._safe_yaw()
                self.driver.set_velocity(-cfg.turn_speed, cfg.turn_speed)
                return
    ...
    def _safe_yaw(self) -> float | None:
        try:
            return self.driver.get_yaw()
        except Exception:
            return None

    def _turn_delta(self) -> float | None:
        """Signed yaw delta since the turn started, wrapped to [-pi, pi]."""
        if self._turn_start_yaw is None:
            return None
        now_yaw = self._safe_yaw()
        if now_yaw is None:
            return None
        d = now_yaw - self._turn_start_yaw
